...
class AuxLossContext(nn.Module):
    ...
    def accumulate(
        ...
        *,
        ...
        """Accumulate routing statistics for one layer and inject z-loss into the main graph.
    ...
    def _cal_tokens_per_expert(self) -> tuple[torch.Tensor, torch.Tensor]:
        """Stack per-layer expert counts and produce both local and globally reduced views.

        The local view is needed by BalancingLossContext's non-global-average branch (per-rank
        scaling); the global view is what the consumer (logging / bias update) wants.
        """
