...
class MoE(BaseModel):
    ...
    def _z_loss_dist_token_count(
        ...
        """Compute the cross-rank non-padding token count needed by the z-loss inline path.
    ...
    def _micro_batch_forward(
        ...
        if self.mtp_block is not None:
            ...
            for depth_idx in range(num_mtp_depths):
                ...
                depth_router_weights = torch.cat([out.router_weights for out in depth_outputs], dim=0)
                depth_router_logits = torch.cat([out.router_logits for out in depth_outputs], dim=0)
                mb0 = mtp_outputs_per_mb[0][depth_idx]
                ...
                mtp_outputs_per_mb[0][depth_idx] = mb0._replace(hidden=mb0_hidden)
            ...
            for micro_batch_idx, (loss_ctx_dict, mtp_outputs) in enumerate(zip(loss_ctx_list, mtp_outputs_per_mb)):
                ...
                for mtp_idx, (mtp_out, mtp_ctx) in enumerate(zip(mtp_outputs, mtp_loss_ctx_list)):
                    mtp_loss, _ = self.lm_head(mtp_out.hidden, cast(MTPLossContext, mtp_ctx))
                    micro_batch_mtp_losses += mtp_loss

                    router_logits_list[micro_batch_idx][f"mtp_layer{mtp_idx}"] = mtp_out.router_logits
                    router_weights_list[micro_batch_idx][f"mtp_layer{mtp_idx}"] = mtp_out.router_weights
